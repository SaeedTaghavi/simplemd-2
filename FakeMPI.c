...
#include "FakeMPI.h"
/*
 *MPI/f90の相性が非常に悪く、cとのリンクもままならないので、mpiの機能のうち、
 *自分が必要な部分を自作する。
 *OpenMosix上での動作を想定しているので、子ノード用のプロセスも親ノード上で走る。
 *
 *平成17年2月9日(水)
...
/*
 *エラーメッセージ出力のための大域変数。stderrにすれば表示される。NULLなら非表示
 */
...
void fakempi_connect( sFakeMPI* fakempi )
{
    ...
    if ( fakempi->myrank == 0 ){
        ...
        fprintf( fakempi_verbose, "Child %d gets connected.( socket %d ) \n", fakempi->myrank, fakempi->mysock );
    }
}
...
void fakempi_finalize( sFakeMPI* fakempi )
{
    ...
    if ( fakempi->myrank == 0 ){
	  ...
	  int ierr;
	  int iopt = 0;
	  waitpid( fakempi->pids[i], &ierr, iopt );
        }
    }
    else{
      //die if it is a child.
      exit(0);
    }
}
